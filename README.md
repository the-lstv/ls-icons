<img width="1017" height="483" alt="header" src="https://github.com/user-attachments/assets/08e7e4a7-c73f-4fee-95fa-aa4c3f14755f" />

# LS.Icons v3
LS.Icons is an open-source icon library designed by Lukas, primarily for [LS](https://github.com/the-lstv/LS), but available for anyone.<br>
This is the 3rd major version (or rather 3rd attempt) after a long time and is **still work in progress**.

So far there is 90 icons (each in 6 styles = 540 icons), and I am working on more.<br> 
They're not perfect (still working things out), but I hope you like them!<br> 
Feel free to suggest icons or give feedback, I'm happy to help.

<img width="391" height="478" alt="Screenshot from 2026-03-29 19-54-41" src="./preview.png" />

## Comming soon
- Icon browser
- Organize styles & enhance iconfont
- More icons (goal is at least 200 by 3.1, then 500, then 1000, then finally 1500/2000+ and I'll be happy)

## Changes compared to 2.0
- Removed outline-thin and fill-bold styles for simplicity.
- Now using a precise keyline to ensure all icons are consistent.
- Updated icon font with woff2.
- An updated style.
- Removed all legacy icons.
- All icons are made from scratch (and variant by variant), so at this moment v2.0 still has more icons than v3.0.

If you have an idea, please let me know and I'll be happy to help you out!

---

<img src=https://cdn.extragon.cloud/file/a771e02dfb37f618.svg> <br>
All icons are hand-made by a human (and just one human, consider supporting please 🥺?).<br>
Licensed under the GPL-3.0 License.
